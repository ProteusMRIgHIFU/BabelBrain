                                    ...
                                    uint gid[[thread_position_in_grid]])
{
    ...
    if (thread_id < info.n_triangles) { // every thread works on specific triangles in its stride
        ...
        #endif
        #ifdef _METAL
        float2 bbox_max_grid = {floor(bbox_max.x / info.unit.y - 0.5), floor(bbox_max.y / info.unit.z - 0.5)};
        float2 bbox_min_grid = {ceil(bbox_min.x / info.unit.y - 0.5), ceil(bbox_min.y / info.unit.z - 0.5)};
        ...
        for (int y = bbox_min_grid.x; y <= bbox_max_grid.x; y++)
        {
            for (int z = bbox_min_grid.y; z <= bbox_max_grid.y; z++)
            {
                ...
                #endif
                #ifdef _METAL
                float2 point = {(y + 0.5)*info.unit.y, (z + 0.5)*info.unit.z};
                #endif
