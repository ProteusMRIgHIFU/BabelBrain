...
class RemoteServerEditDialog(QDialog):
    ...
    def __init__(self, parent=None, server=None):
        ...
        self.setWindowTitle(TR("Remote server"))
        server = server or {}
        form = QFormLayout()
        ...
        self.tokenEdit.setPlaceholderText(TR("(leave empty if the server needs no token)"))
        ...
        # -- TLS / HTTPS --
        self.httpsCheck = QCheckBox(TR("Use HTTPS (TLS)"))
        self.httpsCheck.setChecked(bool(server.get('https', False)))
        self.cafileEdit = QLineEdit(server.get('cafile') or '')
        self.cafileEdit.setPlaceholderText(TR("(CA bundle — only for a private / self-signed cert)"))
        self.clientCertEdit = QLineEdit(server.get('client_cert') or '')
        self.clientCertEdit.setPlaceholderText(TR("(client cert PEM — only for mutual TLS)"))
        self.clientKeyEdit = QLineEdit(server.get('client_key') or '')
        self.clientKeyEdit.setPlaceholderText(TR("(client key PEM — only for mutual TLS)"))
        self.insecureCheck = QCheckBox(TR("Skip certificate verification (testing only — unsafe)"))
...
class RemoteServerManagerDialog(QDialog):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle(TR("Add / remove remote server"))
        self.resize(460, 320)

        self.listWidget = QListWidget()
        self.listWidget.itemDoubleClicked.connect(lambda *_: self._edit())

        addBtn = QPushButton(TR("Add…"))
        editBtn = QPushButton(TR("Edit…"))
        removeBtn = QPushButton(TR("Remove"))
        testBtn = QPushButton(TR("Test"))
        ...
        layout = QVBoxLayout(self)
        layout.addWidget(QLabel(TR("Servers that can run BabelBrain simulations for this client:")))
