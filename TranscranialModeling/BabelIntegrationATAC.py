...
'''
from .ATAC import generate_atac_tx
...
class SimulationConditions(BabelIntegrationCONCAVE_PHASEDARRAY.SimulationConditions):
    ...
    def gen_transducer_geom(self):
        self._Tx=generate_atac_tx(Frequency=self._Frequency,RotationZ=self._RotationZ,FactorEnlarge=self._FactorEnlarge)
        self._TxOrig=generate_atac_tx(Frequency=self._Frequency,RotationZ=self._RotationZ)
