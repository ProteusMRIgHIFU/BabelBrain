...
class SimulationConditions(BabelIntegrationANNULAR_ARRAY.SimulationConditions):
    ...
    '''
    def __init__(self,InDiameters= np.array([0.        , 28.7e-3]), #inner diameter of rings
                      OutDiameters=np.array([28.7e-3 , 40.3e-3]), #outer diameter of rings
                      **kargs): # steering
        Frequency=kargs['Frequency']
        distanceEdgeToEdge = 1500/Frequency/8 # m, distance between the edges of the rings, used to calculate the cross section area entering the domain
        InDiameters[1:]-=distanceEdgeToEdge # m, inner diameter of rings
        OutDiameters[:-1]+=distanceEdgeToEdge # m, outer diameter of rings
